...
def btn(text: str, tone: str = "primary", emoji_index: int = 0) -> str:
    # Reply-keyboard labels cannot receive Bot API entities, so keep readable fallback emojis there.
...
def styled_button(text: str, tone: str = "primary", emoji_index: int = 0, **kwargs: Any) -> InlineKeyboardButton:
    """Create a Telegram Bot API 10.x styled inline button.

    Telegram now supports `style` and `icon_custom_emoji_id` for inline buttons, while older
    python-telegram-bot versions pass unknown Bot API fields through `api_kwargs`.
    """
...
def force_join_keyboard() -> InlineKeyboardMarkup:
    keyboard = [[styled_button(channel["title"], "primary", i, url=channel["invite_link"])] for i, channel in enumerate(force_channels())]
...
def main_keyboard() -> ReplyKeyboardMarkup:
    rows_ = [
        [KeyboardButton("🟢 💎 Create Post"), KeyboardButton("🔵 🎨 Font Changer")],
        [KeyboardButton("🔵 📌 More Bots"), KeyboardButton("🟢 🌈 Share Bot")],
        [KeyboardButton("⚪ 🔗 How to Use")],
        [KeyboardButton("🔴 🛡 Admin Panel")],
    ]
    return ReplyKeyboardMarkup(rows_, resize_keyboard=True, input_field_placeholder="✨ Choose a premium tool")
...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_html(bot_text("<b>Main Menu</b> — use the large keyboard buttons below.", 5), reply_markup=main_keyboard())
...
FONT_MAPS = {
    ...
    "italic": make_font("𝘢𝘣𝘤𝘥𝘦𝘧𝘨𝘩𝘪𝘫𝘬𝘭𝘮𝘯𝘰𝘱𝘲𝘳𝘴𝘵𝘶𝘷𝘸𝘹𝘺𝘻𝘈𝘉𝘊𝘋𝘌𝘍𝘎𝘏𝘐𝘑𝘒𝘓𝘔𝘕𝘖𝘗𝘘𝘙𝘚𝘛𝘜𝘝𝘞𝘟𝘠𝘡0123456789"),
    "bold_italic": make_font("𝙖𝙗𝙘𝙙𝙚𝙛𝙜𝙝𝙞𝙟𝙠𝙡𝙢𝙣𝙤𝙥𝙦𝙧𝙨𝙩𝙪𝙫𝙬𝙭𝙮𝙯𝘼𝘽𝘾𝘿𝙀𝙁𝙂𝙃𝙄𝙅𝙆𝙇𝙈𝙉𝙊𝙋𝙌𝙍𝙎𝙏𝙐𝙑𝙒𝙓𝙔𝙕0123456789"),
    ...
    "mono": make_font("𝚊𝚋𝚌𝚍𝚎𝚏𝚐𝚑𝚒𝚓𝚔𝚕𝚖𝚗𝚘𝚙𝚚𝚛𝚜𝚝𝚞𝚟𝚠𝚡𝚢𝚣𝙰𝙱𝙲𝙳𝙴𝙵𝙶𝙷𝙸𝙹𝙺𝙻𝙼𝙽𝙾𝙿𝚀𝚁𝚂𝚃𝚄𝚅𝚆𝚇𝚈𝚉0123456789"),
    "sans": make_font("𝖺𝖻𝖼𝖽𝖾𝖿𝗀𝗁𝗂𝗃𝗄𝗅𝗆𝗇𝗈𝗉𝗊𝗋𝗌𝗍𝗎𝗏𝗐𝗑𝗒𝗓𝖠𝖡𝖢𝖣𝖤𝖥𝖦𝖧𝖨𝖩𝖪𝖫𝖬𝖭𝖮𝖯𝖰𝖱𝖲𝖳𝖴𝖵𝖶𝖷𝖸𝖹0123456789"),
    "sans_bold": make_font("𝗮𝗯𝗰𝗱𝗲𝗳𝗴𝗵𝗶𝗷𝗸𝗹𝗺𝗻𝗼𝗽𝗾𝗿𝘀𝘁𝘂𝘃𝘄𝘅𝘆𝘇𝗔𝗕𝗖𝗗𝗘𝗙𝗚𝗛𝗜𝗝𝗞𝗟𝗠𝗡𝗢𝗣𝗤𝗥𝗦𝗧𝗨𝗩𝗪𝗫𝗬𝗭0123456789"),
    "wide": make_font("ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ０１２３４５６７８９"),
    "circled": make_font("ⓐⓑⓒⓓⓔⓕⓖⓗⓘⓙⓚⓛⓜⓝⓞⓟⓠⓡⓢⓣⓤⓥⓦⓧⓨⓩⒶⒷⒸⒹⒺⒻⒼⒽⒾⒿⓀⓁⓂⓃⓄⓅⓆⓇⓈⓉⓊⓋⓌⓍⓎⓏ⓪①②③④⑤⑥⑦⑧⑨"),
    "small_caps": make_font("ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘǫʀꜱᴛᴜᴠᴡxʏᴢABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"),
...
async def send_draft(message: Any, d: Draft, preview: bool = False) -> None:
    ...
    await message.reply_html(bot_text(intro, 12))
    if d.media_type == "photo" and d.file_id:
        await message.reply_photo(d.file_id, caption=d.caption, parse_mode=ParseMode.HTML, reply_markup=markup)
    elif d.media_type == "video" and d.file_id:
        await message.reply_video(d.file_id, caption=d.caption, parse_mode=ParseMode.HTML, reply_markup=markup)
    elif d.media_type == "animation" and d.file_id:
        await message.reply_animation(d.file_id, caption=d.caption, parse_mode=ParseMode.HTML, reply_markup=markup)
    elif d.media_type == "document" and d.file_id:
        await message.reply_document(d.file_id, caption=d.caption, parse_mode=ParseMode.HTML, reply_markup=markup)
    else:
        await message.reply_html(d.text or d.caption or "Untitled post", reply_markup=markup)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if "Font Changer" in normalized:
        ...
        return await update.message.reply_html(bot_text("<b>Send any text.</b> I will show 20+ viral font styles with examples.", 13))
    ...
    if "Admin Panel" in normalized or text == "/admin":
        return await admin_panel(update, context)
    if state == "await_post":
        d = store_message_as_draft(update.message, context)
        context.user_data["state"] = None
        await update.message.reply_html(bot_text("<b>Post saved perfectly.</b> Now add premium buttons, preview it, or finish.", 8), reply_markup=builder_keyboard())
        return
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("state") == "await_post":
        ...
        context.user_data["state"] = None
        await update.message.reply_html(bot_text("<b>Post media and caption saved.</b> Add unlimited premium buttons or preview now.", 18), reply_markup=builder_keyboard())
...
async def share_bot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_html(
        bot_text("Share this bot with friends if you like premium post making.", 10),
        reply_markup=InlineKeyboardMarkup([[styled_button("Share Bot", "success", 4, url=f"https://t.me/share/url?url={url}")]]),
    )
